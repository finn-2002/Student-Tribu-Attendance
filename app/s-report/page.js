...
const AttendanceByStudent = () => {
  ...
  useEffect(() => {
    ...
    const fetchStudents = async () => {
      try {
        const response = await fetch('http://localhost/attendance/getStudents.php'); // Make sure to create this endpoint
  ...
  const fetchAttendanceByStudent = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost/attendance/getAttendanceByStudent.php?student_id=${selectedStudent}&from_date=${fromDate}&to_date=${toDate}`
      );
      const result = await response.json();
      if (response.ok) {
        setData(result);
